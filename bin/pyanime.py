...
def choose_servers(servers, needs):
    ...
    logger.warning("No servers found for needs: %s, falling back to: %s", needs, fallback_type)
...
for i, episode in enumerate(selected_episodes, 1):
    logger.info("Processing episode %d/%d: %s", i, len(selected_episodes), episode.get('Title', 'Unknown'))
    
    # Show progress for current episode
    print(f"{hex_to_rgb('#00b3ff', f'[{i}/{len(selected_episodes)}]')} {hex_to_rgb('#fc861e', f'Processing Episode {episode["No"]}: {episode["Title"]}')}")
    
    try:
        logger.info("Extracting servers for episode %s", episode.get('Episode ID', 'Unknown'))
        print(f"  {hex_to_rgb('#00b3ff', 'Extracting servers...')}")
        servers = serverextractor(episode)
        logger.info("Found %d servers for episode", len(servers))
        
        selected_servers = choose_servers(servers, needs)
        if not selected_servers:
            logger.warning("No servers found for episode %s. Skipping.", episode.get('Episode ID', 'Unknown'))
            print(f"  {hex_to_rgb('#ff0000', 'No servers found. Skipping.')}")
            continue
        
        logger.info("Getting media streams for episode")
        print(f"  {hex_to_rgb('#00b3ff', 'Getting media streams...')}")
        media = streams(selected_servers[0], episode)
        logger.info("Parsing m3u8 for episode")
        print(f"  {hex_to_rgb('#00b3ff', 'Parsing media...')}")
        media, name = m3u8_parsing(media)
        
        if media is None:
            logger.error("Failed to parse m3u8 for episode %s", episode.get('Episode ID', 'Unknown'))
            print(f"  {hex_to_rgb('#ff0000', 'Failed to parse media. Skipping.')}")
            continue
        
        separator("=", f"Downloading Episode {episode['No']}: {name}", "info")
        logger.info("Starting download for episode: %s", name)
        print(f"{hex_to_rgb('#00ff00', 'Starting download...')}")
        
        code = downloading(media, f"{episode['No']}. {name}", anime_details['title'])
        
        if code == 1:
            logger.error("Download failed for episode %s", episode.get('Episode ID', 'Unknown'))
            print()
            print(f"{hex_to_rgb('#FF0000', 'Download failed!')} Check {hex_to_rgb('#73FF2E', 'pyanime.log')} for details.")
            print("Exiting...")
            print()
            separator("=")
            exit(1)
        elif code == 0:
            logger.info("Download completed successfully for episode %s", episode.get('Episode ID', 'Unknown'))
            print(f"{hex_to_rgb('#00ff00', '✓ Download completed successfully!')}")
            print(f"{hex_to_rgb('#00b3ff', f'File saved in: .animecache/{anime_details["title"]}/')}")
            separator("=")
            print()
    
    except Exception as e:
        logger.error("Unexpected error processing episode %s: %s", episode.get('Episode ID', 'Unknown'), str(e), exc_info=True)
        print(f"  {hex_to_rgb('#ff0000', f'Error: {str(e)}')}")
        continue

logger.info("Download process completed")
print()
print(f"{hex_to_rgb('#00ff00', '🎉 All downloads completed!')}")
separator('=', "Download Complete", "info")
print(f"{hex_to_rgb('#00ff00', f'Successfully downloaded {len(selected_episodes)} episode(s)')}")
print(f"{hex_to_rgb('#00b3ff', f'Files saved in: .animecache/{anime_details["title"]}/')}")
print(f"{hex_to_rgb('#75e64c', 'Thank you for using pyanime!')}")
print()
print(f"{hex_to_rgb('#00ff00', 'Done')}")
print("Exiting...")
print()
separator("=")
